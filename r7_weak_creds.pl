...
# 05.04.2016
# 
# Modified by Olivier Pinchard: added CIFS support
# 11.Sept.2019 
#
#
#
...
sub cifs {
    ...
    my $id;
    if ( $realm eq '*none*' ) {
        $id = "cmty-cifs-default-account-$username-password-$password";
    }
    else {
        $id = "cmty-cifs-default-account-$username-password-$password-$realm";
    }
    ...
    $xml
        .= "<Vulnerability id=\"$id\" added=\"$ymd\" modified=\"$ymd\" version=\"2.0\">\n";
    $xml
        .= "   <name>Default-Weak CIFS account: $username password \"$password\"</name>\n";
    $xml .= "   <severity>10</severity>\n";
    $xml .= "   <pci severity=\"5\"/>\n";
    $xml .= "   <Tags>\n";
    $xml .= "       <tag>Default Account</tag>\n";
    ...
    $xml
        .= "   <p>The $username account uses a password of \"$password\".\n";
    $xml
        .= "The CIFS service was found to have a weak or default account configuration with the user ID \"$username\" and password \"$password\". This would allow anyone to log into the machine and take complete control of the system.</p>\n";
    $xml .= "   </Description>\n";
    $xml .= " <Solutions>\n";
    $xml .= "   <Solution id=\"$id\" time=\"15m\">\n";
    $xml
        .= "       <summary>Fix Default-Weak CIFS account: $username password \"$password\"</summary>\n";
    $xml .= "       <workaround>\n";
    $xml
        .= "<p>Set a more secure login password. A good\n";
    $xml 
        .= "   password should consist of a mix of lower- and upper-case characters,\n";
    $xml
        .= "   numbers, and punctuation and should be at least 8 characters long.</p>\n";
    ...
    foreach my $realm (@realms) {
        ...
        foreach my $username (@usernames) {
            ...
            foreach my $password (@passwords) {
                ...
                elsif ( $service eq 'cifs' ) {
                    print "cifs check generated for: $username/$password\n";
                    cifs( $username, $password, $realm );
                }
                else { }
            }
        }
